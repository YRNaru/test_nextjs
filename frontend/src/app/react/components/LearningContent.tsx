...
export default function LearningContent({ sectionData }: LearningContentProps) {
  ...
  if (!sectionData) {
    return <p data-oid="4b3cp0l">セクションが見つかりません。</p>;
  }
  ...
  const renderExamples = () => {
    if (
      ...
      return <p data-oid="sq:yy3:">このセクションには例がありません。</p>;
    }
    return (sectionData.examples as BasicExample[]).map((example) => (
      <ExampleCard key={example.id} example={example} data-oid="-hpm083" />
    ));
  };
  ...
  return (
    ...
    >
      <h2
        className="text-[var(--card-text)] mb-4 text-3xl font-bold bg-gradient-to-br from-[#3498db] to-[#2ecc71] bg-clip-text text-transparent leading-tight tracking-tight"
        data-oid="tom1-7i"
      >
        {sectionData.title}
      </h2>
      ...
      >
        <h3
          className="text-[var(--card-text)] mb-6 text-2xl font-bold bg-gradient-to-br from-[#3498db] to-[#2ecc71] bg-clip-text text-transparent leading-tight tracking-tight"
          data-oid="0-l68l7"
        >
          重要なポイント
        </h3>
        <ul className="list-none p-0" data-oid="-p8_y8u">
      ...
      >
        <h3
          className="text-[var(--card-text)] mb-6 text-2xl font-bold bg-gradient-to-br from-[#3498db] to-[#9b59b6] bg-clip-text text-transparent leading-tight tracking-tight"
          data-oid="go_rw3o"
        >
          例
        </h3>
