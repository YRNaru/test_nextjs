  ...
  examples: [
    {
      ...
      description:
        "React DOMのクライアントAPIを使って、ブラウザのDOMノードにReactコンポーネントを描画します。",
...
root.render(<App />);`,
      ...
      explanation:
        "createRootでルートを作成し、render()でコンポーネントを描画します。これがReactアプリのエントリーポイントです。",
    ...
    },
    {
      ...
      name: "純粋な関数コンポーネント",
      description:
        "コンポーネントは純粋関数として定義し、同じpropsに対して常に同じ結果を返すべきです。",
...
<Greeting name="World" />`,
  ...
  return <div>{random}</div>;
      ...
      explanation:
        "純粋なコンポーネントは同じ入力に対して常に同じ出力を返し、副作用を持ちません。これによりReactが自動的に最適化できます。",
  ...
  <p>本文</p>
      ...
      explanation:
        "Fragmentを使うことで、余計なDOMノードを増やさずに複数の要素を返せます。JSXでは単一のルート要素が必要です。",
    ...
    },
    {
      ...
      description:
        "開発者はコンポーネントを関数として定義するだけで、実際に呼び出すのはReactです。",
      example: `// コンポーネントを定義
function Button() {
...
Button(); // これはReactのルールに反する`,
      explanation:
        "コンポーネントはJSXで記述することで、Reactが必要なタイミングで呼び出します。これにより最適なレンダリングタイミングをReactが制御できます。",
...
export const basicPractice: PracticeQuestion[] = [
  ...
  },
  {
    ...
    correctAnswer: 0,
    explanation:
      "コンポーネントは純粋関数として定義し、同じ入力に対して常に同じ出力を返すべきです。",
    type: "component",
  },
  {
    id: "q3",
...
export const componentData: LearningSection = {
  ...
  description:
    "ReactではUIをコンポーネント単位で分割して構築します。本節では、関数コンポーネントを定義して表示させる基本的な手順を、Hello.tsxという簡単な例を通じて説明しています。",
  keyPoints: [
...
export const componentPractice: PracticeQuestion[] = [
  {
    ...
    correctAnswer: 1,
    explanation:
      "Reactの関数コンポーネントはconst Hello = () => <div>Hello</div>; のように定義します。",
    type: "component",
  },
  {
    id: "q2",
...
export const propsData: LearningSection = {
  ...
  title: "Props（プロップス）",
  description:
    "Reactでは、propsを使うことで、親コンポーネントから子コンポーネントに値を渡すことができます。本節では、propsの基本的な使い方、データの受け渡し方法、制約、および再利用性の向上について解説されています。",
  keyPoints: [
...
export const apiData: LearningSection = {
  ...
  title: "API",
  description:
    "Reactの組み込みAPIは、コンポーネントの定義やリソースへのアクセスに役立つ機能を提供します。createContext、lazy、memo、startTransition、actなどのAPIがあり、それぞれ異なる用途で使用されます。",
  keyPoints: [
...
export const contextData: LearningSection = {
  ...
  description:
    "ReactのContextは、コンポーネント間でグローバルにデータを共有するための仕組みです。通常のpropsでは親から子への明示的な受け渡しが必要ですが、Contextを使うことで複数階層をまたいだデータ共有が可能になります。",
  keyPoints: [
  ...
  examples: [
    {
      ...
      description:
        "stateカテゴリの代表であるuseStateを使って、ユーザー操作に応じた表示変更を行います。",
...
return (
  <>
    ...
    <p>{artworks[index].title}</p>
      ...
      explanation:
        "useStateは現在の値と更新関数を返し、更新するとコンポーネントが再レンダーされます。",
...
const Toolbar = () => {
  ...
  return <button className={theme}>テーマを適用</button>;
      ...
      explanation:
        "useContextを使うとpropsを渡し続けなくても、コンポーネントツリーのどこからでも共有データにアクセスできます。",
    ...
    },
    {
      ...
      description:
        "useEffectはDOM描画のあとでチャット接続やイベント購読など外部システムとの同期を行います。",
      example: `useEffect(() => {
  ...
  doSomething();
      ...
      explanation:
        "依存配列に指定した値が変化したときのみ副作用が再実行され、戻り値の関数でリソースを解放できます。",
...
return (
  <>
    ...
    <h2>{artwork.title}</h2>
      ...
      explanation:
        "useStateは配列で現在値と更新関数を返し、ユーザー操作に合わせて再レンダーします。",
...
return <button onClick={() => setCount((c) => c + 1)}>クリック数: {count}</button>;`,
      ...
      explanation:
        "更新関数は前回の値を受け取り、新しい値を返します。非同期やバッチ更新でも安全です。",
...
export const eventData: LearningSection = {
  ...
  title: "イベント処理の基本",
  description:
    "Reactでは、HTMLのイベント（クリック、変更、送信など）をJavaScriptの関数で制御できます。この節では、イベントハンドラの基本的な記述方法やイベントオブジェクトの活用方法について解説されています。",
  keyPoints: [
...
export const eventPractice: PracticeQuestion[] = [
  ...
  },
  {
    ...
    code: "",
    options: ["e: MouseEvent", "e: React.MouseEvent<HTMLButtonElement>", "e: Event", "e: any"],
    correctAnswer: 1,
  ...
  },
  {
    ...
    code: "",
    options: ["useState", "setState", "setCount(新しい値)", "count = 新しい値"],
    correctAnswer: 2,
...
export const inputEventData: LearningSection = {
  ...
  title: "イベントで入力欄を操作する",
  description:
    "この節では、Reactのイベントハンドリングを通して、テキスト入力欄（input要素）を操作する方法を学びます。具体的には、入力欄の値を取得・更新し、それに連動したUI表示を行う方法が説明されています。",
  keyPoints: [
...
const [count, dispatch] = useReducer(reducer, 0);`,
      ...
      explanation:
        "dispatchを呼ぶとreducerが実行され、新しいcountが返ってコンポーネントが再レンダーされます。",
...
const [state, dispatch] = useReducer(reducer, loadFromStorage(), init);`,
      ...
      explanation:
        "init関数は初回にだけ呼ばれるため、ストレージからの読み込みや変換処理を安全に行えます。",
  ...
  examples: [
    {
      ...
      description:
        "React.memoでラップしたButtonが、依存するstateが変わらない限り再レンダーされません。",
...
return (
  <>
    ...
    <p>{count}</p>
      ...
      explanation:
        "useCallbackでコールバックを安定させると、props比較で差分がなくなり再レンダーを抑えられます。",
    ...
    },
    {
      ...
      description:
        "useCallbackを使ってonChangeを再利用できるようにし、コンポーネントの再レンダーを抑えます。",
      example: `const [text, setText] = useState('');
...
return <input value={text} onChange={handleChange} />;`,
      ...
      explanation:
        "安定したコールバックを渡すことで、入力コンポーネントが余計な再レンダーを行わずに済みます。",
...
const format = useCallback(
  ...
  [locale]
      ...
      explanation:
        "依存配列にlocaleを含めることで、ロケールが変わったときだけ新しいフォーマッタを生成できます。",
...
const filteredItems = useMemo(() => {
  ...
  return tasks.filter((task) => task.completed).length;
      ...
      explanation:
        "派生データをuseMemoでまとめて計算すると、複雑なUIでも描画コストを抑えられます。",
...
export const useCallbackPractice: PracticeQuestion[] = [
  ...
  },
  {
    ...
    code: "",
    options: ["関数が再生成される", "値が再計算される", "副作用が発生する", "何も起きない"],
    correctAnswer: 0,
...
export const useEffectData: LearningSection = {
  ...
  return () => connection.disconnect();
      ...
      explanation:
        "依存配列にroomIdを指定すると部屋が変わったときだけエフェクトが再実行され、戻り値でリソースを解放できます。",
...
useEffect(() => {
  document.title = \`Click count: \${count}\`;
      ...
      explanation:
        "副作用をレンダー外に移動することで、描画ロジックとブラウザAPIの同期ロジックを分離できます。",
    ...
    },
    {
      ...
      description:
        "スクロール位置を追跡するためにウィンドウイベントを購読し、クリーンアップで解除します。",
      example: `useEffect(() => {
  ...
  return () => window.removeEventListener('scroll', onScroll);
      ...
      explanation:
        "クリーンアップを返すことで、再レンダーやアンマウント時に不要なリスナーが残りません。",
...
export const useContextPractice: PracticeQuestion[] = [
  ...
  },
  {
    ...
    correctAnswer: 0,
    explanation:
      "Contextの値が変わると、それを参照しているコンポーネントが再レンダリングされます。",
    type: "hook",
  },
  {
    id: "q4",
...
export const useLayoutEffectPractice: PracticeQuestion[] = [
  {
    ...
    code: "",
    options: ["描画前のDOM操作やレイアウト調整", "副作用の管理", "値のメモ化", "イベント登録"],
    correctAnswer: 0,
  ...
  },
  {
    ...
    code: "",
    options: ["実行タイミング", "API", "依存配列の書き方", "違いはない"],
    correctAnswer: 0,
  ...
  },
  {
    ...
    code: "",
    options: ["UI描画が遅れる", "パフォーマンスが上がる", "何も起きない", "副作用が減る"],
    correctAnswer: 0,
  ...
  },
  {
    ...
    code: "",
    options: ["useEffect", "useLayoutEffect", "どちらでもよい", "必ず両方使う"],
    correctAnswer: 0,
    explanation:
      "通常はuseEffectを使い、チラつき防止やレイアウト調整が必要な場合のみuseLayoutEffectを使います。",
    type: "hook",
  },
];
...
return <p>前回: {prevCount.current ?? 'なし'} / 現在: {count}</p>;`,
    ...
    },
    {
      ...
      description:
        "forwardRefした子コンポーネントからメソッドを公開し、親が呼び出せるようにします。",
...
return <Dialog ref={dialogRef} />;`,
      ...
      explanation:
        "useImperativeHandleを使うと、内部実装は隠しつつ親に必要なコマンドだけを提供できます。",
...
export const customHookData: LearningSection = {
  ...
  return online;
      ...
      explanation:
        "Hook内でuseStateとuseEffectを組み合わせ、接続状態を再利用可能なAPIとして提供します。",
  ...
  return { value, onChange, reset: () => setValue(initial) };
      ...
      explanation:
        "状態・イベント・デバッグ表示をまとめることで、フォームの入力欄を簡潔に扱えます。",
  ...
  return data;
      ...
      explanation:
        "カスタムフックにまとめることで、データ取得とキャンセル処理を複数箇所で安全に再利用できます。",
...
export const useMemoPractice: PracticeQuestion[] = [
  ...
  },
  {
    ...
    correctAnswer: 0,
    explanation:
      "再計算コストが高い処理や、大きな配列・オブジェクトを毎回生成したくない場合にuseMemoが有効です。",
    type: "hook",
  },
  {
    id: "q4",
    ...
    correctAnswer: 0,
    explanation:
      "useMemoは値をメモ化し、useCallbackは関数をメモ化します。用途を区別して使い分けます。",
    type: "hook",
  },
];
