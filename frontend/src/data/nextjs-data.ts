...
export const topicDetails: Record<TopicKey, TopicDetail> = {
  routing: {
    ...
    title: "ファイルベースルーティング",
    summary:
      "File-based Routingは、Next.js 13以降のApp Routerで採用されているルーティングシステムです。ファイルシステムの構造に基づいて自動的にルートが生成される仕組みで、直感的で強力なルーティングシステムを提供します。",
    features: [
  ...
  rendering: {
    ...
    title: "レンダリング",
    summary:
      "Next.jsでは用途に応じて様々なレンダリング手法（SSG/SSR/CSRなど）を選択できます。下のタブで切り替えて詳細を確認しましょう。",
    features: [],
    useCases: [],
    cautions: [],
    notes: [],
  ...
  typescript: {
    ...
    title: "TypeScript対応",
    summary:
      "Next.jsはTypeScriptを標準サポートしており、型安全な開発ができます。ts/tsxファイルを使うだけで型推論や型チェックが有効になります。",
    features: [
    ...
    notes: ["Next.jsプロジェクトでts/tsxファイルを作成するだけでTypeScriptが有効になります。"],
  },
  optimize: {
    icon: "🚀",
    title: "自動最適化",
    summary:
      "Next.jsは画像やフォント、バンドルサイズなどを自動で最適化し、パフォーマンスやユーザー体験を向上させます。",
    features: [
    ...
    notes: ["next/imageやnext/fontは公式ドキュメントも参照してください。"],
  },
};
...
export const subRenderingDetails: Record<SubRenderingKey, TopicDetail> = {
  ssg: {
    ...
    title: "静的サイト生成（SSG）",
    summary:
      "SSG（Static Site Generation）は、ビルド時にあらかじめHTMLを生成し、静的ファイルとして配信するレンダリング手法です。",
    features: [
  ...
  ssr: {
    ...
    summary:
      "SSR（Server Side Rendering）は、ページにアクセスがあるたびにサーバー側でデータを取得・HTMLを生成し、それをクライアントに返す方式。getServerSideProps 関数を使い、リクエストのたびに実行。サーバーで描画されたHTMLがレスポンスとして返される。",
    features: [
  ...
  csr: {
    ...
    summary:
      "CSR（Client Side Rendering）は、ビルド時にデータを取得せず、ブラウザ上で初期表示した後に非同期でデータを取得し、画面を更新する描画方式です。",
    features: [
  ...
  isr: {
    ...
    summary:
      "ISR（Incremental Static Regeneration）は、SSG（静的サイト生成）の拡張版で、ビルド時に生成された静的ページに有効期限（寿命）を設定することで、一定時間後に自動的に再生成できる手法です。getStaticProps に revalidate を指定することで、一定時間ごとにバックグラウンドで新しいページが生成される。ユーザーには古いページが即時に返されるが、次回アクセス時には最新データが反映された新しいページが表示される。",
    features: [
