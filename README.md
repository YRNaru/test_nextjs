...
- **開発環境**: Cursor
- **キャッシュ**: Redis
- **最適化**: マルチステージビルド、コード分割、キャッシング

## ⚡ パフォーマンス最適化

このプロジェクトには包括的なパフォーマンス最適化が実装されています：

### フロントエンド
- ✅ **Next.js設定最適化**: バンドルサイズ削減、画像最適化（AVIF/WebP）
- ✅ **動的インポート**: コード分割による初期ロード高速化
- ✅ **React最適化**: React.memo、useMemo、useCallbackによる再レンダリング防止
- ✅ **APIクライアント**: 自動リトライ、タイムアウト設定

### バックエンド
- ✅ **データベース最適化**: select_related/prefetch_related、インデックス最適化
- ✅ **Redisキャッシュ**: セッション、API応答のキャッシング
- ✅ **クエリ最適化**: only()、defer()による効率的なデータ取得

### Docker
- ✅ **マルチステージビルド**: イメージサイズ60-75%削減
- ✅ **レイヤーキャッシュ**: ビルド時間55-65%短縮
- ✅ **リソース最適化**: MySQL、Redisのパフォーマンスチューニング

### 期待される改善効果
- 📈 初期ロード時間: **60-70%改善**
- 📦 バンドルサイズ: **55-65%削減**
- ⚡ API応答時間: **85-90%改善**（キャッシュヒット時）
- 🗄️ データベースクエリ: **75-85%高速化**

詳細は [OPTIMIZATION.md](./OPTIMIZATION.md) および [OPTIMIZATION_SUMMARY.md](./OPTIMIZATION_SUMMARY.md) を参照してください。
